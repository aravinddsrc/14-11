...
{
    public class ViewAllProjectsController : Controller
    {
        //
        // GET: /ViewAllProjects/
        ...
        [HttpPost]
        public ActionResult EditProject(Projects data, List<int> TechList1, List<int> ORMList1, List<int> DBList1, List<int> ThirdPartyList1, List<int> SourceControlsList1)
        {
            ...
            try
            {
                ...
                //}
                result = db.Projects.Where(o => o.ProjectID == ID)
                                        .Select(o => new Projects
                                        {
                                            ProjectID = o.ProjectID,
                                            ProjectName = o.ProjectName,
                ...
                if (ModelState.IsValid)
                {
                    // DSRCManagementSystemEntities1 db = new DSRCManagementSystemEntities1();
                    {
                        ...
                        if (data.Phases != null)
                        {
                            ...
                            foreach (string phase in data.Phases)
                            {
                        ...
                        if (boolProjectTime)
                        {
                            ...
                            {
                                var check = db.EmailTemplates.Where(x => x.TemplatePurpose == "Schedule Change Fixed Price").Select(o => o.EmailTemplateID).FirstOrDefault();
                                var folder = db.EmailTemplates.Where(o => o.TemplatePurpose == "Schedule Change Fixed Price").Select(x => x.TemplatePath).FirstOrDefault();
                                if ((check != null) && (check != 0))
                                {
                                    ...
                                    htmlProjectStatus = htmlProjectStatus.Replace("#ManagedBy", data.ManagedResources);
                                ...
                                {
                                   // string ServerName = WebConfigurationManager.AppSettings["SeverName"];
                                    ExceptionHandlingController.TemplateMissing("Schedule Change Fixed Price", folder, ServerName);
                                }
                            }
                        }
                        //if (boolProjectPlan)
                        ...
                        if (data.ProjectType.ToString() == "Fixed Price")
                        {
                            ...
                            var check = db.EmailTemplates.Where(x => x.TemplatePurpose == "Project Plan Change Fixed Price").Select(o => o.EmailTemplateID).FirstOrDefault();
                            var folder = db.EmailTemplates.Where(o => o.TemplatePurpose == "Project Plan Change Fixed Price").Select(x => x.TemplatePath).FirstOrDefault();
                            if ((check != null) && (check != 0))
                            {
                                ...
                                htmlProjectStatus = htmlProjectStatus.Replace("#ComapanyName", company);
                            ...
                            {
                              //  string ServerName = WebConfigurationManager.AppSettings["SeverName"];
                                ExceptionHandlingController.TemplateMissing("Project Plan Change Fixed Price", folder, ServerName);
                            }
                        }
                        //}
        ...
        {
            var result = new ProjectRAGStatus();
            try
            {
        ...
        public ActionResult ProjectSummary()
        {
            ...
            using (DSRCManagementSystemEntities1 db = new DSRCManagementSystemEntities1())
            {
                var result = db.Projects.Where(p => p.IsDeleted != true || p.IsDeleted == null && p.IsActive == true).Select(x => x).ToList();
                ...
                List<string> CommentedBy = new List<string>();

                foreach (var item in commentedlist)
                {
                    if (item != 0 && item != null)
                    {
                        var Name = db.Users.Where(x => x.UserID == item).Select(o => o.FirstName + " " + (o.LastName ?? "")).FirstOrDefault();
                        CommentedBy.Add(Name);
                    }
                    else
                    {
                        CommentedBy.Add("-");
                ...
                //string MessageBody = MailBody + Tablebody + MessageBody2;
                 ...
                 var check = db.EmailTemplates.Where(x => x.TemplatePurpose == "Project Summary").Select(o => o.EmailTemplateID).FirstOrDefault(); 
                     ...
                     if ((check != null) && (check != 0))
                     {
                         ...
                         objProjectSummary.To = ViewAllProjectsController.GetUserEmailAddress(db, objProjectSummary.To);
                         objProjectSummary.CC = ViewAllProjectsController.GetUserEmailAddress(db, objProjectSummary.CC);
                         if (objProjectSummary.BCC != "")
                         {
                             objProjectSummary.BCC = ViewAllProjectsController.GetUserEmailAddress(db, objProjectSummary.BCC);
                         }
                ...
                using (DSRCManagementSystemEntities1 db = new DSRCManagementSystemEntities1())
                {
                    ...
                    if (!res)
                    {
                        ...
                        var objProjectStatus = (from p in db.EmailPurposes.Where(x => x.EmailPurposeName == "Add New Project")
                         ...
                         var check = db.EmailTemplates.Where(x => x.TemplatePurpose == "Add New Fixed Project").Select(o => o.EmailTemplateID).FirstOrDefault(); 
                     var folder= db.EmailTemplates.Where(o=> o.TemplatePurpose == "Add New Fixed Project").Select(x=> x.TemplatePath).FirstOrDefault();
                     if ((check != null) && (check != 0))
                     {
                         ...
                         htmlProjectStatus = htmlProjectStatus.Replace("#ManagedBy", projectmodel.ManagedResources);
                         //htmlProjectStatus = htmlProjectStatus.Replace("#ManagdBy", projectmodel.ManagedResources);
                         htmlProjectStatus = htmlProjectStatus.Replace("#ServerName",ServerName);
                         htmlProjectStatus = htmlProjectStatus.Replace("#CompanyName", company);
                         if (projectmodel.ProjectType.ToString() == "1")
                     ...
                     {
                        // string ServerName = WebConfigurationManager.AppSettings["SeverName"];
                         ExceptionHandlingController.TemplateMissing("Add New Fixed Project", folder, ServerName);
                     }
        ...
        [HttpGet]
        public ActionResult ViewMetrics(int ProjectID)
        {
            DSRCManagementSystemEntities1 objdb = new DSRCManagementSystemEntities1();
        ...
        public ActionResult CommentsSummary(int projectID)
        {
            DSRCManagementSystemEntities1 db = new DSRCManagementSystemEntities1();
            {
        ...
        [HttpGet]
        public ActionResult ResourcesTabWithAssignProject(int Id)
        {
            DSRCManagementSystemEntities1 objdb = new DSRCManagementSystemEntities1();
                ...
                using (DSRCManagementSystemEntities1 db = new DSRCManagementSystemEntities1())
                {
                    ...
                    if (memberObj.UserId != null && memberObj.ProjectId != 0)
                    {
                     ...
                     if ((check != null) && (check != 0))
                     {
                         ...
                         objAssignProject.CC = ViewAllProjectsController.GetUserEmail(db, objAssignProject.CC);
                         if (objAssignProject.BCC != "")
                         {
                             objAssignProject.BCC = ViewAllProjectsController.GetUserEmail(db, objAssignProject.BCC);
                         }
                        ...
                        // string ServerName = WebConfigurationManager.AppSettings["SeverName"];
                         ...
                         if (ServerName  != "http://win2012srv:88/")
                         {
                             ...
                             string CCMailId = "ramesh.s@dsrc.co.in";
                             string BCCMailId = "ramesh.s@dsrc.co.in";
                     ...
                     if ((check != null) && (check != 0))
                     {
                         ...
                         objAssignProjectDelete.CC = ViewAllProjectsController.GetUserEmail(db, objAssignProjectDelete.CC);
                         if (objAssignProjectDelete.BCC != "")
                         {
                             objAssignProjectDelete.BCC = ViewAllProjectsController.GetUserEmail(db, objAssignProjectDelete.BCC);
                         }
        ...
        public ActionResult ProjectDetails(int Id)
        {
            ...
            ProjectDetails.Members = (
                ...
                */
                                      ...
                                      join usr in db.Users.Where(u => u.IsActive == true && u.UserStatus!=6) on usrproj.UserID equals usr.UserID
        ...
        [HttpGet]
        public ActionResult Tab(int Id, int TypeID)
        {
